...
tft = tft_config.config(tft_config.TALL, buffer_size=64*64*2)
...
def main():
    ...
    def update_missiles():
        ...
        for missile in list(missiles):                          # for each missile
            missile.draw(gc9a01.BLACK)                          # erase old missile
            if missile.counter > 0:                             # if counter > 0 missile is active
                ...
                for roid in list(roids):                        # for each roid
                    if missile.collision(roid):                 # check if missile collides with roid
                        roid.draw(gc9a01.BLACK)                 # erase the roid
                ...
                if missile.counter > 0:                         # if the missile has life left
                    missile.draw(gc9a01.WHITE)                  # draw missile
    ...
    def update_roids():
        ...
        for roid in roids:                  # for each roid, erase, move then draw
            roid.draw(gc9a01.BLACK)
            roid.move()
            roid.draw(gc9a01.WHITE)

            if roid.collision(ship):        # check for roid/ship collision
                ship.draw(gc9a01.BLACK)     # erase ship
    ...
    while True:
        ...
        if ship_alive:
            ...
            if buttons.thrust and buttons.fire.value() == 0 and len(missiles) < missile_max:
