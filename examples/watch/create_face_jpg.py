...
def maon():
    ...
    fnt = ImageFont.truetype(
        "./LibreBaskerville-Regular.ttf", 18
    )  # get a font of an appropriate size
    ...
    for minute in range(1, 60):
        ...
        d.line(
            [ofs + x1 + cx, y1 + cx, ofs + x2 + cx, y2 + cx], width=1, fill="#000000"
        )
    ...
    for hour in range(1, 13):
        ...
        # draw the hour tick
        d.line(
            [ofs + x1 + cx, y1 + cx, ofs + x2 + cx, y2 + cx], width=5, fill="#ff0000"
        )
        ...
        d.text(
            (
                ofs + x + cx - ((size[2] + size[0] >> 1)),
                y + cx - ((size[3] + size[1]) >> 1),
            ),
            str(hour),
            font=fnt,
            fill=(0, 0, 0),
